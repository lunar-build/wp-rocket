...
use WP_Rocket\Engine\Common\Queue\AbstractBGProcessQueue;
 ...
 * @version 3.11.0
 */
class BGQueue extends AbstractBGProcessQueue {
	...
	public function schedule_pending_jobs_cron( int $interval ) {
		add_filter( 'cron_schedules', function ( $schedules ) use ( $interval ) {
			$schedules['rucss_custom_schedule'] = [
				'interval' => $interval,
				'display'  => __( 'RUCSS Custom Schedule', 'rocket' ),
			];
			return $schedules;
		} );
		return $this->schedule_recurring( time(), 'rucss_custom_schedule', $this->pending_job_cron );
	}
